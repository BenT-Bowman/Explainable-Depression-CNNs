...
class legacy(nn.Module):
    ...
    def transformer_path(self, x):
        ...
        x = self.t_batchnorm1(x)
        x = self.activation(x)
        x = F.avg_pool2d(x, (1,4)) # TODO: Try less aggressive or not at all.

        x = self.t_conv2(x)
        x = self.t_batchnorm2(x)
        x = self.dropout2(x)
        x = self.activation(x)
        x = F.avg_pool2d(x, (1,8)) # TODO: Try less aggressive or not at all.
